...
> A **key** is a piece of information, usually a string of numbers or letters, which, when used in a <ins>cipher</ins>, can encode or decode data.
...
* **Asymmetric** - The cipher uses separate keys for encryption and decryption. These are known as <ins>public</ins> and <ins>private keys</ins>, respectively.

Coupled with keys, most ciphers also utilize **initialization vectors**.

> An **initialization vector** (or IV for short) is a variable used in several ciphers to <ins>randomize</ins> the encryption, thus producing distinct ciphertexts even if the same plaintext is encrypted multiple times.

## Task 1: Frequency Analysis

Our first task was to decipher a message encrypted using a **monoalphabetic cipher**.

> A **monoalphaebtic cipher** is a substitution cipher where each symbol in the original message is <ins>consistently</ins> replaced with another symbol to form the ciphertext.

This type of cipher is vulnerable to a technique called **frequency analysis**.

> **Frequency analysis** consists in counting the occurrence of each <ins>symbol</ins> in a ciphertext. It is based on the fact that, in any given piece of text, certain symbols and combinations of symbols occur with varying frequencies.
...
**Note:** As recommended by the guide, we ensured the symbols we substituted were in <ins>uppercase</ins> so we could more easily notice our changes.
...
> A **stream cipher** is a <ins>symmetric key</ins> cipher where plaintext symbols are combined with a pseudorandom cipher symbol stream, known as a <ins>keystream</ins>.
...
**Note:** A <ins>keystream</ins> is generated by supplying a <ins>key</ins> to a pseudorandom bit generator.

### Block ciphers

> A **block cipher** is a <ins>symmetric key</ins> cipher that operates on fixed-length groups of bits, called <ins>blocks</ins>.
...
|---------------|--------------|
| Takes one <ins>symbol</ins> of plaintext at a time. | Takes one <ins>block</ins> of plaintext at a time.|
...
> A **cipher mode** is an algorithm that describes how to repeatedly apply a <ins>cipher</ins>'s single-block operation to securely transform amounts of data larger than a block.
...
> Electronic Code Book (or **ECB** for short) is a simple encryption mode wherein each block of the original message is encrypted <ins>separately</ins>.
...
> Cipher Block Chaining (or **CBC** for short) is an encryption mode wherein each block of the original message is <ins>XORed</ins> with the previous ciphertext block before beign encrypted. The first block is XORed with an <ins>initialization vector</ins>.

![Alt text](images/10-7.png)

This mode guarantees each ciphered block depends on all plaintext blocks processed up to that point. In addition, the **initialization vector** ensures the uniqueness of each message, meaning the same plaintext encrypted twice using the same key will <ins>not</ins> yield the same ciphertext.
